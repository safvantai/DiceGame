...
public class DiceStarsZoneExtension extends BaseZoneAdaptor {
    
    private IZone izone;
    
    DiceStarsZoneExtension(IZone izone){  
    ...
    public void handleAddUserRequest(final IUser user, final String authData, final HandlingResult result){
        System.out.println("User is came...");
...
//        }
    ...
    public void handleCreateRoomRequest(IUser user, IRoom room, HandlingResult result)
    {
        ...
        if(room.isTurnBased()){    //&& room.getMaxUsers() == 3     
            
            room.setAdaptor(new DiceStarsRoomExtensionLeisureGame(izone, (ITurnBasedRoom)room));
            
        }
//        else if(room.isTurnBased() && room.getMaxUsers()==3){
        
//            room.setAdaptor(new DiceStarsRoomExtensionTournament(izone, (ITurnBasedRoom)room));
...
//        }
    ...
    public void onUserPaused(IUser user){
        ...
        if(user.getLocation().getMaxUsers()==2){
            ...
            DiceStarsRoomExtensionLeisureGame extension = (DiceStarsRoomExtensionLeisureGame)user.getLocation().getAdaptor();
            
            extension.onUserPaused(user);
            
        }else if(user.getLocation().getMaxUsers()==3){
            
            DiceStarsRoomExtensionTournament extension = (DiceStarsRoomExtensionTournament)user.getLocation().getAdaptor();
    ...
    {
        System.out.println("Resume user: "+user.getName());
